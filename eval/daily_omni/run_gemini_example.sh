...
# 
# 使用方法:
#   1. 设置你的 API Key 和文件路径（在下方配置区域）
#   2. 运行: bash eval/daily_omni/run_gemini_example.sh
#   或
#   ./eval/daily_omni/run_gemini_example.sh

# ========== 配置区域 ==========
# 请修改以下配置为你的实际路径和 API Key

# Gemini API Key（必需）
export GEMINI_API_KEY="AIzaSyDjCRT3t8okZw3iMFvesGKzNHRiOTOV5-Q"
export GOOGLE_API_KEY="AIzaSyDjCRT3t8okZw3iMFvesGKzNHRiOTOV5-Q"

# 选择 Gemini 模式: gemini_av (音频+视频) 或 gemini_visual (仅视频)
export MODEL_TYPE=${MODEL_TYPE:-"gemini_av"}
...
if [ "$DEBUG_MODE" = "1" ]; then
    ...
    echo ""
    CMD="python -m debugpy --connect $DEBUG_PORT eval/daily_omni/run_evaluation.py --model $MODEL_TYPE --mode $EXECUTION_MODE"
else
...
# 打印配置信息
echo "=========================================="
echo "Daily-Omni Benchmark 评估 (Gemini API)"
echo "=========================================="
